    ...
    try {
      ...
      for (const user of users) {
        ...
        for (const job of jobs) {
          ...
          let targetDay = 0;
          if (daysSinceSent >= 6) targetDay = 6;
          else if (daysSinceSent >= 3) targetDay = 3;
...
- Output ONLY the body of the email, starting with exactly "Dear Hiring Manager at ${job.company},". No subject, no sign-off, no markdown blocks.`;
          ...
          try {
            ...
            logger.info('[Cron] Successfully sent follow-up', { company: job.company, day: targetDay });
          } catch (err) {
            logger.error('[Cron] Failed to generate/send follow-up', { error: err.message, company: job.company });
          }
        }
      }
