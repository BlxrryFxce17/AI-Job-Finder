...
function sanitizeAiInstructions(instructions) {
  ...
  // Using a simpler regex to avoid linting issues with escaped characters
  const allowedChars = new RegExp('[^\\w\\s.,!?@#$%&*()\\-_=+[\\]{}|;:\'"<>~`]', 'g');
  ...
  if (!discoveredEmail && config.hunterApiKey) {
    try {
      const hRes = await axios.get(
        `https://api.hunter.io/v2/domain-search?domain=${domain}&api_key=${config.hunterApiKey}&department=hr`
      );
